...
/** Global market template composer — available to all prop firms. */
export function GlobalMarketForm() {
  ...
  async function create() {
    ...
    setMessage(null);
    try {
      const response = await fetch("/api/platform/markets", {
        method: "POST",
      ...
      if (!response.ok) {
        setMessage({ ok: false, text: body.error ?? "Could not create template" });
        return;
      }
      setMessage({ ok: true, text: `Template published: "${body.template.question}"` });
      setQuestion("");
      setProbability(50);
      router.refresh();
    } catch {
