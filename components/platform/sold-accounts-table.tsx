...
const providerTones: Record<string, "accent" | "up" | "down" | "warn" | "neutral"> = {
  kalshi: "accent",
  polymarket: "up",
  internal: "down",
  sp500_dynamic: "warn",
...
function providerLabel(provider: string): string {
  if (provider === "sp500_dynamic") return "S&P 500";
  return provider;
}

function linkedMarketsLabel(row: SoldAccountRow): string {
  if (row.provider === "sp500_dynamic") {
    const n = row.sp500_tickers?.length ?? 0;
    return n ? `${n} tickers` : "—";
  }
  if (row.kalshi_market_tickers?.length) {
    return `${row.kalshi_market_tickers.length} linked`;
  }
  return "—";
}
...
export function SoldAccountsTable() {
  ...
  // Ensure filter always offers sp500_dynamic even before any sales land.
  if (!providers.includes("sp500_dynamic")) {
    providers.push("sp500_dynamic");
    providers.sort();
  }
  const filtered =
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <table className="w-full min-w-[1180px] text-sm">
          <thead>
            <tr className="border-b border-edge text-left text-[11px] uppercase tracking-wide text-faint">
              ...
              <th className="pb-2 pr-4 font-medium">Linked markets</th>
          ...
          <tbody>
            {filtered.map((row) => (
              <tr key={row.id} className="border-b border-edge/60 last:border-0 hover:bg-surface-2/60">
                ...
                <td className="py-3 pr-4">
                  <Badge tone={providerTones[row.provider] ?? "accent"}>
                ...
                <td className="py-3 pr-4 text-xs text-faint">{linkedMarketsLabel(row)}</td>
                <td className="py-3 pr-4">
                  {row.email_sent ? (
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-muted">No sold accounts for this provider.</p>
      ) : null}
    </div>
  );
}
