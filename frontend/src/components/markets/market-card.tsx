...
export function MarketCard({ market }: { market: Market }) {
  ...
  return (
    <Card className="group flex flex-col transition-colors hover:border-edge-strong">
      <Link href={`/markets/${market.id}`} className="flex flex-1 flex-col p-4 sm:p-5">
        ...
        <div className="mt-4 flex items-end justify-between gap-3">
          <div>
            ...
            <div className="mt-0.5 flex items-baseline gap-2">
              <span className="tabular text-2xl font-bold tracking-tight">{yesPct}%</span>
        ...
        <div className="mt-3 flex h-1.5 w-full gap-0.5 overflow-hidden rounded-full">
          <div className="rounded-l-full bg-up" style={{ width: `${yesPct}%` }} />
          <div className="flex-1 rounded-r-full bg-down/70" />
        </div>
        <div className="mt-1.5 flex justify-between text-[11px] font-medium">
          <span className="text-up">Yes {formatCents(market.yesPrice)}</span>
          <span className="text-down">No {formatCents(1 - market.yesPrice)}</span>
        </div>

        <div className="mt-4 flex items-center justify-between border-t border-edge pt-3 text-[11px] text-faint">
