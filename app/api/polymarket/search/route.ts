...
export async function GET(request: NextRequest) {
  ...
  const markets = await searchPolymarketMarkets(q, refresh);
  const filtered = applyFilters(markets, { category, status, activeOnly: active });
  const sorted = sortMarkets(filtered, sort);
  const payload = paginate(sorted, page, pageSize);

  return NextResponse.json({ ...payload, query: q });
}
